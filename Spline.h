...
#define M_PI 3.1415926535897932
        ...
        public:
            ...
            enum bd_type {
                first_deriv = 1,//指定两个端点的一阶倒数为某个固定值
                second_deriv = 2,//两个端点的二阶倒数为0（ Natural Spline）
                not_a_knot = 3//第一个插值点的三阶导数值等于第二个点的三阶导数值，最后第一个点的三阶导数值等于倒数第二个点的三阶导数值，输入的X坐标要大于=4个
            };
        ...
        public:
            ...
            // at both ends, i.e. natural splines,默认是Natural Spline
            ...
            // adjust coefficients so that the spline becomes piecewise(分段) monotonic(单调的无变化的)
            // where possible
            //   this is done by adjusting slopes(斜率) at grid points by a non-negative
